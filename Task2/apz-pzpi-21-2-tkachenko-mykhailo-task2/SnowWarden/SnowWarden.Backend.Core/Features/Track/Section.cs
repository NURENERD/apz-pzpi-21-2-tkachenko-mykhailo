...
public class Section : IDbEntity
{
	...
	{
		this.Information.Danger = danger;
		this.AggregateDangerIndex = (int)danger;
	}
	...
	public void RecalculateDangerIndex(IotRequestMetadata metadata)
	{
		...
		foreach (SectionFactor sectionFactor in this.DeterminingFactors.Values)
		{
			metadata.TryGetValue(sectionFactor.Key, out int value);
			//bool indexValid = double.TryParse(value, out double index);
