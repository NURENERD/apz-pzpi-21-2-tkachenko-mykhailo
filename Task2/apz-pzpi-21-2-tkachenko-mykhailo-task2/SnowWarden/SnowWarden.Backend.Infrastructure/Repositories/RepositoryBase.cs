...
public abstract class RepositoryBase<TEntity> : IRepository<TEntity> where TEntity : class, IDbEntity
{
	...
	protected virtual async Task ValidateSingleAttachment<TAttachment>(TEntity source, Func<TEntity, TAttachment?> validationDelegate, Func<TEntity, int> foreignKeySelector) where TAttachment : class, IDbEntity, new()
	{
		...
		_ = await Context.Set<TAttachment>().FindAsync(attachment.Id)
			?? throw new InvalidAttachementEntityException(attachment);
	}
	...
	protected virtual async Task ValidateManyAttachments<TAttachment>(TEntity source, Func<TEntity, ICollection<TAttachment>> validationDelegate) where TAttachment : class, IDbEntity
	{
		...
		List<TAttachment> notValid = await Context.Set<TAttachment>()
			.Where(att =>
				!attachments.Select(at => at.Id).Contains(att.Id))
			.ToListAsync();
		if (notValid.Any()) throw new InvalidAttachementEntityException(notValid.First());
	}
}